...
export const env_value = (key: string): string => {
    if (!key || !(key in process.env)) throw new Error(`Invalid key. (key=${key})`);
...
export namespace Logger {
    const ENDPOINT = new WebhookClient({ url: `https://discord.com/api/webhooks/${env_value('APP_LOGGER_URL')}` }); 
