...
export const execute = async (/** @type {ChatInputCommandInteraction} */ i) => {
    const tier = i.guild.members.resolve(i.user).roles.cache.find(r => Object.values(DM_ROLES).includes(r.id));
    if (!tier) return await i.reply({ content: `You may do this as a Marked person, or as a ${roleMention(DM_ROLES.GHOST)}`, ephemeral: true });
    ...
    , kt = await i.guild.roles.fetch(DM_ROLES.KISMET) || (_ => { basic_log(Error('Kismet Marked role retrieval returned undefined.')); return null; })();
    setTimeout(async _ => {
        try {
            ...
            , result = d6_roll(tier.id, kt && kt.members.size > 0)
