...
import { DM_ROLES } from '../constants.js';
import { Common, Logger } from '../utils.js';
...
export default {
    ...
    execute: async (/** @type {ChatInputCommandInteraction} */ i) => {
        const tier = i.guild.members.resolve(i.user).roles.cache.find(r => [ DM_ROLES.DEATH, DM_ROLES.CANCR, DM_ROLES.SCARL, DM_ROLES.KISMT, DM_ROLES.GREYD, DM_ROLES.GHOST ].includes(r.id));
        if (!tier) return await i.reply({ content: `You may do this as a ${roleMention(DM_ROLES.MARKD)} person, or as a ${roleMention(DM_ROLES.GHOST)}`, ephemeral: true });
        ...
        setTimeout(async () => {
            try {
                ...
                const result = Common.random([DM_ROLES.GREYD, DM_ROLES.GHOST].includes(tier.id) && 1 || 5).map(n => chances.find(p => p.max >= n).value)
